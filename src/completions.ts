...
class BufferCompletionOption implements CompletionOption {
    ...
    render(highlight: boolean = false): void {
        ...
        if (this.pre) html.childNodes[0].textContent = ` ${this.pre.padEnd(2)} `
...
async function updateIfCurrent(epochref: any, asyncFunc: Function, updateFunc: Function, ...args: any[]): Promise<any> {
    ...
    if (epoch === epochref) return updateFunc(res)
...
function completionFromTab(tab: browser.tabs.Tab, prev: boolean = false): BufferCompletionOption {
    let pre: string = ""
    if (tab.active) pre += "%"
    else if (prev) pre += "#"
    if (tab.pinned) pre += "@"
...
export function getBuffersFromTabs(tabs: browser.tabs.Tab[]): BufferCompletionSource {
    ...
    for (const tab of tabs) { buffers.push(completionFromTab(tab, tab === prev)) }
